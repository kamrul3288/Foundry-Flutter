...
class ShellRouteKeysMainScreen extends StatelessWidget {
  const ShellRouteKeysMainScreen({super.key});
...
class _HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Shell Keys Home'),
        centerTitle: true,
        leading: IconButton(
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const Text(
              'This example uses unique keys for ShellRoutes to distinguish between them.',
...
class _SettingsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Settings (Keys)'),
        centerTitle: true,
        leading: IconButton(
          icon: const Icon(Icons.arrow_back),
          onPressed: () => Navigator.of(context, rootNavigator: true).pop(),
...
class _LoginScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Shell Keys Login'),
        centerTitle: true,
        leading: IconButton(
          icon: const Icon(Icons.arrow_back),
          onPressed: () => Navigator.of(context, rootNavigator: true).pop(),
      ...
      body: Center(
        child: Padding(
          ...
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              const Icon(Icons.vpn_key, size: 80, color: Colors.indigo),
              const SizedBox(height: 24),
              const Text(
                'Access Granted',
                style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold),
              ...
              const SizedBox(height: 16),
              const Text(
                'Demonstrating ShellRoute logic with specialized keys.',
                textAlign: TextAlign.center,
                style: TextStyle(color: Colors.grey),
              ),
              const SizedBox(height: 32),
              FilledButton.icon(
                ...
                label: const Text('Enter Application'),
              ),
            ],
          ),
        ),
